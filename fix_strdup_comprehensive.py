#!/usr/bin/env python3
"""
Comprehensive strdup() safety fix script for Myco codebase.
This script identifies and fixes all strdup() calls to ensure null pointer safety.
"""
...
def analyze_strdup_call(strdup_call):
    ...
    # Check if it already has null safety
    if '?' in argument and ':' in argument:
        return {
            'needs_fix': False,
            'reason': 'Already has ternary null check',
            'argument': argument
        }
    ...
    if argument.startswith('"') and argument.endswith('"'):
        return {
            'needs_fix': True,
            'reason': 'String literal - needs null check',
            'argument': argument,
            'fix_type': 'literal'
        }
...
def fix_strdup_call(line, strdup_call, analysis):
    ...
    if analysis['fix_type'] == 'literal':
        # For string literals, we can safely use them
        return line  # String literals are safe
    
    elif analysis['fix_type'] == 'variable':
